...
def test_form_by_index_name_or_id():
    ...
    assert (
        str(excinfo.value)
        == """No form with name="unknown" found. Found forms with these names: ['one', 'two']"""
    )
    ...
    assert (
        str(excinfo.value)
        == """No form with id="unknown" found. Found forms with these ids: ['id1', 'id2']"""
    )
...
def test_form_data__update_not_possible_if_dict_is_frozen():
    ...
    with pytest.raises(
        ValueError,
        match=r"Key 'unknown_key' is not in the dict. Available: my_input",
    ):
